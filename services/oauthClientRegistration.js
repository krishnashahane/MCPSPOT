...
import { mutateOAuthSettings, persistClientCredentials, persistTokens, } from './oauthSettingsStore.js';
// Cache for registered clients to avoid re-registering on every restart
const registeredClients = new Map();
export const removeRegisteredClient = (serverName) => {
 ...
 */
export const fetchProtectedResourceMetadata = async (resourceMetadataUrl) => {
    try {
 ...
 */
export const fetchScopesFromServer = async (serverUrl) => {
    try {
        ...
        console.log(`Could not fetch scopes from server (this is normal if not using OAuth discovery): ${error instanceof Error ? error.message : String(error)}`);
        return undefined;
    }
};
/**
 ...
 */
export const autoDetectOAuthConfig = async (wwwAuthenticateHeader, serverUrl) => {
    try {
 ...
 */
export const discoverIssuer = async (issuerUrl, clientId = 'mcpspot-temp', clientSecret) => {
    try {
        ...
        throw new Error(`OAuth issuer discovery failed: ${error instanceof Error ? error.message : String(error)}`);
    }
};
/**
 ...
 */
export const registerClient = async (serverName, serverConfig, autoDetectedIssuer, autoDetectedScopes) => {
    ...
    try {
        ...
        if (issuerUrl) {
            ...
            throw new Error(`Cannot register OAuth client: no issuer URL available. Either provide 'issuer' in configuration or ensure server returns proper 401 with WWW-Authenticate header.`);
        }
        ...
        const clientMetadata = {
            client_name: metadata.client_name || `MCPSpot - ${serverName}`,
        ...
        const clientInfo = {
            config,
            clientId,
            clientSecret: config.client_secret, // Access client secret if available
            registrationAccessToken: config.registrationAccessToken,
            registrationClientUri: config.registrationClientUri,
            expiresAt: config.client_secret_expires_at
                ? config.client_secret_expires_at * 1000
                : undefined,
            metadata: config,
        };
 ...
 */
export const getAuthorizationUrl = async (serverName, serverConfig, clientInfo, redirectUri, state, codeVerifier) => {
    try {
 ...
 */
export const exchangeCodeForToken = async (serverName, serverConfig, clientInfo, currentUrl, codeVerifier) => {
    try {
        ...
        const tokens = await client.authorizationCodeGrant(clientInfo.config, new URL(currentUrl), { expectedState: undefined }, // State is already validated
        tokenParams);
 ...
 */
export const refreshAccessToken = async (serverName, serverConfig, clientInfo, refreshToken) => {
    try {
 ...
 */
export const calculateCodeChallenge = async (codeVerifier) => {
    return client.calculatePKCECodeChallenge(codeVerifier);
};
/**
 * Get registered client info from cache
 */
export const getRegisteredClient = (serverName) => {
 ...
 */
export const initializeOAuthForServer = async (serverName, serverConfig, autoDetectedIssuer, autoDetectedScopes) => {
    if (!serverConfig.oauth) {
        return null;
    }
    // Check if dynamic registration should be attempted
    const shouldAttemptRegistration = autoDetectedIssuer || // Auto-detected from 401 response
    ...
    if (shouldAttemptRegistration) {
        try {
            ...
            const clientInfo = await registerClient(serverName, serverConfig, autoDetectedIssuer, autoDetectedScopes);
            return clientInfo;
        }
        catch (error) {
    ...
    if (serverConfig.oauth.clientId) {
        ...
        try {
            ...
            const clientAuth = serverConfig.oauth.clientSecret
                ...
                : client.None();
            const config = await client.discovery(serverUrl, serverConfig.oauth.clientId, undefined, clientAuth);
            const clientInfo = {
                config,
...
//# sourceMappingURL=oauthClientRegistration.js.map