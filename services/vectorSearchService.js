...
import { truncateToTokenLimit, getModelDefaultTokenLimit, } from '../utils/tokenTruncation.js';
...
const getAzureOpenAIConfig = (smartRoutingConfig) => {
    return {
...
const generateAzureOpenAIEmbedding = async (text, smartRoutingConfig) => {
    ...
    const url = `${endpoint}/openai/deployments/${encodeURIComponent(azureConfig.embeddingDeployment)}/embeddings?api-version=${encodeURIComponent(azureConfig.apiVersion)}`;
    ...
    const azureMaxTokens = smartRoutingConfig.embeddingMaxTokens ?? getModelDefaultTokenLimit(embeddingModel);
 ...
 */
export async function createVectorIndex(dataSource, dimensions, tableName = 'vector_embeddings', columnName = 'embedding') {
    ...
    if (dimensions <= HALFVEC_MAX_DIMENSIONS) {
      ...
      `);
            ...
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            const isHalfvecNotSupported = errorMessage.includes('halfvec') ||
            ...
            if (isHalfvecNotSupported) {
                ...
                console.warn('═══════════════════════════════════════════════════════════════════════════');
                console.warn(`  Your embeddings have ${dimensions} dimensions, which requires halfvec support.`);
                console.warn('');
                console.warn('  pgvector dimension limits:');
                console.warn(`  - vector type: max ${VECTOR_MAX_DIMENSIONS} dimensions`);
                console.warn(`  - halfvec type: max ${HALFVEC_MAX_DIMENSIONS} dimensions (pgvector 0.7.0+)`);
                console.warn('');
                console.warn('  RECOMMENDATIONS:');
                console.warn('  1. Upgrade pgvector to >= 0.7.0 for halfvec support');
                console.warn('  2. Or use a smaller embedding model:');
                console.warn('     - text-embedding-3-small (1536 dimensions) instead of text-embedding-3-large');
...
const supportBase64Embeddings = async (baseURL = '') => {
...
async function generateEmbedding(text) {
    ...
    if (provider === 'azure_openai') {
        ...
        try {
            ...
            console.warn(`Azure OpenAI embeddings request failed (status=${status ?? 'unknown'}). Falling back to local embeddings.`);
            console.warn(`Azure embedding config: endpoint=${azureConfig.endpoint || 'missing'}, deployment=${azureConfig.embeddingDeployment || 'missing'}, apiVersion=${azureConfig.apiVersion || 'missing'}`);
    ...
    const rawMaxTokens = smartRoutingConfig.embeddingMaxTokens ?? getModelDefaultTokenLimit(config.embeddingModel);
    const maxTokens = isSiliconFlow ? Math.floor(rawMaxTokens * TOKEN_SAFETY_FACTOR) : rawMaxTokens;
    const truncatedText = await truncateToTokenLimit(text, maxTokens, config.embeddingModel, config.apiKey);
    ...
    try {
        ...
        if (encodingFormat === 'base64' && typeof response.data[0].embedding === 'string') {
            const embeddingBase64Str = response.data[0].embedding;
        ...
        const message = error instanceof Error ? error.message : String(error);
        console.warn(`OpenAI embeddings request failed (status=${status ?? 'unknown'}). Falling back to local embeddings.`);
        console.warn(`Embedding config: baseURL=${config.baseURL || 'default'}, model=${config.embeddingModel || 'default'}`);
        console.warn(`Embedding error: ${message}`);
        return generateFallbackEmbedding(text);
    }
}
