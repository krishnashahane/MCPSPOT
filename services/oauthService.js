...
export { getRegisteredClient, getAuthorizationUrl, exchangeCodeForToken, generateCodeVerifier, calculateCodeChallenge, autoDetectOAuthConfig, parseWWWAuthenticateHeader, fetchProtectedResourceMetadata, } from './oauthClientRegistration.js';
let oauthProvider = null;
let oauthRouter = null;
/**
 * Initialize OAuth provider from system configuration
 */
export const initOAuthProvider = async () => {
    ...
    try {
        ...
        console.log('OAuth endpoints configured: authorization, token' +
 ...
 */
export const getServerOAuthToken = async (serverName) => {
    ...
    if (serverConfig.oauth.dynamicRegistration?.enabled) {
        try {
            ...
            if (serverConfig.oauth.refreshToken) {
                try {
                    const tokens = await refreshAccessToken(serverName, serverConfig, clientInfo, serverConfig.oauth.refreshToken);
 ...
 */
export const addOAuthHeader = async (serverName, headers) => {
 ...
 */
export const initializeAllOAuthClients = async () => {
    const serverDao = getServerDao();
    ...
    for (const serverConfig of allServers) {
        ...
        if (serverConfig.oauth?.dynamicRegistration?.enabled === true) {
            registrationPromises.push(initializeOAuthForServer(serverName, serverConfig)
                .then((clientInfo) => {
                if (clientInfo) {
                ...
                .catch((error) => {
                console.error(`✗ Error pre-registering OAuth client for server ${serverName}:`, error.message);
            }));
        }
    }
    ...
    if (registrationPromises.length > 0) {
        ...
        console.log(`OAuth client pre-registration completed for ${registrationPromises.length} server(s)`);
    }
    else {
