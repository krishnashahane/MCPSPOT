 ...
 */
const generateProxychainsConfig = (serverName, proxyConfig) => {
...
`;
    ...
    const tempDir = path.join(os.tmpdir(), 'mcpspot-proxychains');
 ...
 */
const wrapWithProxychains = (serverName, command, args, proxyConfig) => {
    ...
    if (process.platform === 'win32') {
        console.warn(`[${serverName}] proxychains4 proxy is not supported on Windows, ignoring proxy configuration`);
        return { command, args };
    }
    // Find proxychains4 binary
    const proxychains4Path = findProxychains4();
    if (!proxychains4Path) {
        console.warn(`[${serverName}] proxychains4 not found on system, install it with: apt install proxychains4 (Debian/Ubuntu) or brew install proxychains-ng (macOS)`);
        return { command, args };
    }
    ...
    console.log(`[${serverName}] Using proxychains4 proxy: ${proxyConfig.type || 'socks5'}://${proxyConfig.host}:${proxyConfig.port}`);
    return {
...
export const notifyToolChanged = async (name) => {
...
export const syncToolEmbedding = async (serverName, toolName) => {
...
export const createRequestContextAwareFetch = (baseFetch, passthroughHeaderNames) => {
        ...
        for (const conf of allServers) {
            ...
            client
                ...
                if (!dataError) {
                    ...
                    setupClientKeepAlive(serverInfo, expandedConf).catch((e) => console.warn(`Keepalive setup failed for ${name}:`, e));
                }
                else {
                    serverInfo.status = 'disconnected';
...
export const registerAllTools = async (isInit, serverName) => {
...
export const getServersInfo = async (page, limit, user) => {
    ...
    const combinedServerInfos = [...serverInfos];
...
// Remove server
export const removeServer = async (name) => {
    ...
    for (const serverInfo of filteredServerInfos) {
        if (serverInfo.prompts && serverInfo.prompts.length > 0) {
            ...
            if (serverConfig && serverConfig.prompts) {
                enabledPrompts = serverInfo.prompts.filter((prompt) => {
                    const promptConfig = serverConfig.prompts?.[prompt.name];
...
export const createMcpServer = (name, version, group) => {
    ...
    const server = new Server({ name: serverName, version }, { capabilities: { tools: {}, prompts: {}, resources: {} } });
