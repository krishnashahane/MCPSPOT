...
const isBearerKeyAllowedForRequest = async (req, key) => {
    const paramValue = req.params?.group;
    ...
    try {
        ...
        if (matchedGroup) {
            ...
            if (key.accessType === 'servers') {
                ...
                const groupServerNames = matchedGroup.servers.map((server) => typeof server === 'string' ? server : server.name);
            ...
            if (key.accessType === 'custom') {
                ...
                const groupAllowed = allowedGroups.includes(matchedGroup.name) || allowedGroups.includes(matchedGroup.id);
                if (groupAllowed) {
                    return true;
                }
        ...
        console.warn(`Bearer key access denied: parameter '${paramValue}' does not match any group or server`);
        return false;
    }
    catch (error) {
...
const validateBearerAuth = async (req) => {
    ...
    if (!enableBearerAuth) {
        ...
        if (matchingKey) {
            ...
            if (allowed) {
                console.log(`Bearer key recognized (auth disabled): id=${matchingKey.id}, name=${matchingKey.name}, accessType=${matchingKey.accessType}`);
                return { valid: true, keyId: matchingKey.id, keyName: matchingKey.name };
            }
            console.warn(`Bearer key matched but rejected due to scope restrictions (auth disabled): id=${matchingKey.id}, name=${matchingKey.name}, accessType=${matchingKey.accessType}`);
            return { valid: true };
        }
    ...
    if (enabledKeys.length === 0) {
        ...
        console.warn('Bearer authentication failed: no configured keys and token is not a valid OAuth token');
    ...
    if (matchingKey) {
        const allowed = await isBearerKeyAllowedForRequest(req, matchingKey);
        if (!allowed) {
            console.warn(`Bearer key rejected due to scope restrictions: id=${matchingKey.id}, name=${matchingKey.name}, accessType=${matchingKey.accessType}`);
            return { valid: false, reason: 'invalid' };
        }
        console.log(`Bearer key authenticated: id=${matchingKey.id}, name=${matchingKey.name}, accessType=${matchingKey.accessType}`);
        return { valid: true, keyId: matchingKey.id, keyName: matchingKey.name };
    }
    const oauthUser = await resolveOAuthUserFromToken(token);
    if (oauthUser) {
...
const attachUserContextFromBearer = (result, res) => {
...
const buildResourceMetadataUrl = (req) => {
...
const sendBearerAuthError = (req, res, reason) => {
    const errorDescription = reason === 'missing' ? 'No authorization provided' : 'Invalid bearer token';
...
export const handleSseConnection = async (req, res) => {
    ...
    console.log(`New SSE connection established: ${transport.sessionId} with group: ${group || 'global'}${username ? ` for user: ${username}` : ''}`);
...
export const handleSseMessage = async (req, res) => {
    ...
    const preSessionId = req.query.sessionId;
...
async function createSessionWithId(sessionId, group, username) {
...
async function createNewSession(group, username) {
...
export const handleMcpPostRequest = async (req, res) => {
    ...
    const group = req.params.group;
    const body = req.body;
    console.log(`Handling MCP post request for sessionId: ${sessionId} and group: ${group}${username ? ` for user: ${username}` : ''} with body: ${JSON.stringify(body)}`);
    ...
    if (sessionId && transportInfo) {
        ...
        console.log(`[SESSION SKIP] Ignoring session-less notification request (method: ${req.body.method})${username ? ` for user: ${username}` : ''}`);
        res.status(200).end();
        return;
    }
    else {
