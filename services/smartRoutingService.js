...
let serverInfosRef = [];
let getServerInfosFn = () => serverInfosRef;
let filterToolsByConfigFn;
let filterToolsByGroupFn;
/**
 * Initialize the smart routing service with references to mcpService functions
 */
export const initSmartRoutingService = (getServerInfos, filterToolsByConfig, filterToolsByGroup) => {
 ...
 */
export const getSmartRoutingTools = async (group) => {
    ...
    let availableServers = getServerInfos().filter((server) => server.status === 'connected' && server.enabled !== false);
...
Available servers: ${serversList}`,
            ...
            name: 'describe_tool',
            description: 'STEP 2 of 3: Use this tool AFTER search_tools to get the full parameter schema for a specific tool. This provides the complete inputSchema needed to correctly invoke the tool with call_tool.\n\nWorkflow: search_tools → describe_tool → call_tool',
            inputSchema: {
                type: 'object',
                properties: {
                    toolName: {
            ...
            inputSchema: {
                ...
                properties: {
                    ...
                    arguments: {
                        type: 'object',
                        description: 'The arguments to pass to the tool based on its inputSchema from describe_tool (optional if tool requires no arguments)',
                    },
                },
                required: ['toolName'],
            },
 ...
 */
export const handleSearchToolsRequest = async (query, limit, sessionId) => {
    ...
    const resolvedTools = await Promise.all(searchResults.map(async (result) => {
        // Find the server in serverInfos
        const server = getServerInfos().find((serverInfo) => serverInfo.name === result.serverName &&
    ...
    const filterResults = await Promise.all(resolvedTools.map(async (tool) => {
        if (tool.name) {
            const serverName = tool.serverName;
            if (serverName) {
                let tools = await filterToolsByConfigFn(serverName, [tool]);
                if (tools.length === 0) {
                    return false;
                }
 ...
 */
export const handleDescribeToolRequest = async (toolName, sessionId) => {
        ...
        content: [
            {
                ...
                text: JSON.stringify({
                    ...
                    metadata: {
                        message: 'The specified tool was not found. Use search_tools to discover available tools.',
                    },
                }),
            },
        ],
 ...
 */
export const isSmartRoutingGroup = (group) => {
    return group === '$smart' || (group?.startsWith('$smart/') ?? false);
};
//# sourceMappingURL=smartRoutingService.js.map