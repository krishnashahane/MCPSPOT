...
export async function generateOpenAPISpec(options = {}) {
    ...
    let filteredServers = serverInfos.filter((server) => server.status === 'connected' &&
    ...
    // Get server URL
    const baseUrl = options.serverUrl || systemConfig?.install?.baseUrl || `http://localhost:${config.port}`;
    const serverUrl = `${baseUrl}${config.basePath}/api`;
    // Generate OpenAPI document
    const openApiDoc = {
        openapi: '3.0.3',
        info: {
            title: options.title || 'MCPSpot API',
            description: options.description ||
                'OpenAPI specification for MCP tools managed by MCPSpot. This enables integration with OpenWebUI and other OpenAPI-compatible systems.',
            version: options.version || '1.0.0',
            contact: {
                name: 'MCPSpot',
                url: 'https://github.com/krishnashahane/mcpspot',
            },
            license: {
                name: 'ISC',
                url: 'https://github.com/krishnashahane/mcpspot/blob/main/LICENSE',
            },
        },
        servers: [
            {
                url: serverUrl,
                description: 'MCPSpot API Server',
            },
        ],
        paths,
        components: {
