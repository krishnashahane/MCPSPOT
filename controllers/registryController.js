 ...
 */
export const getAllRegistryServers = async (req, res) => {
    try {
        ...
        const errorMessage = error instanceof Error ? error.message : 'Failed to fetch registry servers';
        res.status(500).json({
            success: false,
 ...
 * Proxies the request to avoid CORS issues in the frontend
 */
export const getRegistryServerVersions = async (req, res) => {
    try {
        ...
        const errorMessage = error instanceof Error ? error.message : 'Failed to fetch registry server versions';
        res.status(500).json({
            success: false,
            message: errorMessage,
        });
 ...
 * Proxies the request to avoid CORS issues in the frontend
 */
export const getRegistryServerVersion = async (req, res) => {
    try {
        ...
        const response = await fetch(`${REGISTRY_BASE_URL}/servers/${encodedName}/versions/${encodedVersion}`, {
            headers: {
                Accept: 'application/json, application/problem+json',
            },
        });
        ...
        const errorMessage = error instanceof Error ? error.message : 'Failed to fetch registry server version';
        res.status(500).json({
            success: false,
            message: errorMessage,
        });
