import { getAllGroups, getGroupByIdOrName, createGroup, updateGroup, updateGroupServers, deleteGroup, addServerToGroup, removeServerFromGroup, getServerConfigInGroup, getServerConfigsInGroup, updateServerToolsInGroup, } from '../services/groupService.js';
// Get all groups
...
export const batchCreateGroups = async (req, res) => {
    try {
        ...
        const validateGroupConfig = (group) => {
...
export const updateGroupServersBatch = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        for (const server of servers) {
            if (typeof server === 'object' && server !== null) {
                ...
                if (server.tools &&
                    server.tools !== 'all' &&
                    (!Array.isArray(server.tools) ||
                        !server.tools.every((tool) => typeof tool === 'string'))) {
                    res.status(400).json({
                        success: false,
...
export const addServerToExistingGroup = async (req, res) => {
    try {
        const { id } = req.params;
...
export const removeServerFromExistingGroup = async (req, res) => {
    try {
...
export const getGroupServers = async (req, res) => {
    try {
        const { id } = req.params;
        if (!id) {
            res.status(400).json({
...
export const getGroupServerConfigs = async (req, res) => {
    try {
        const { id } = req.params;
        if (!id) {
            res.status(400).json({
...
export const getGroupServerConfig = async (req, res) => {
    try {
        const { id, serverName } = req.params;
        if (!id || !serverName) {
            res.status(400).json({
...
export const updateGroupServerTools = async (req, res) => {
    try {
        const { id, serverName } = req.params;
