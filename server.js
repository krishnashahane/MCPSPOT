...
export class AppServer {
    ...
    async initialize() {
        try {
            ...
            // Initialize OAuth authorization server (for MCPSpot's own OAuth)
            ...
            initUpstreamServers()
                ...
                this.app.get(`${this.basePath}/sse/:group(.*)?`, sseUserContextMiddleware, (req, res) => handleSseConnection(req, res));
                this.app.post(`${this.basePath}/messages`, sseUserContextMiddleware, handleSseMessage);
                this.app.post(`${this.basePath}/mcp/:group(.*)?`, sseUserContextMiddleware, handleMcpPostRequest);
                this.app.get(`${this.basePath}/mcp/:group(.*)?`, sseUserContextMiddleware, handleMcpOtherRequest);
                this.app.delete(`${this.basePath}/mcp/:group(.*)?`, sseUserContextMiddleware, handleMcpOtherRequest);
                // User-scoped routes with user context middleware
                this.app.get(`${this.basePath}/:user/sse/:group(.*)?`, sseUserContextMiddleware, (req, res) => handleSseConnection(req, res));
                this.app.post(`${this.basePath}/:user/messages`, sseUserContextMiddleware, handleSseMessage);
                this.app.post(`${this.basePath}/:user/mcp/:group(.*)?`, sseUserContextMiddleware, handleMcpPostRequest);
                this.app.get(`${this.basePath}/:user/mcp/:group(.*)?`, sseUserContextMiddleware, handleMcpOtherRequest);
                this.app.delete(`${this.basePath}/:user/mcp/:group(.*)?`, sseUserContextMiddleware, handleMcpOtherRequest);
            })
                .catch((error) => {
    ...
    findAndServeFrontend() {
        ...
        if (this.frontendPath) {
            ...
            this.app.get(rootPath, (_req, res) => {
                res
                    .status(404)
                    .send('Frontend not found. MCPSpot API is running, but the UI is not available.');
            });
        }
    }
    start() {
        this.server = this.app.listen(this.port, () => {
            ...
            if (this.frontendPath) {
                console.log(`Open http://localhost:${this.port} in your browser to access MCPSpot UI`);
            }
            else {
                console.log(`MCPSpot API is running on http://localhost:${this.port}, but the UI is not available`);
            }
        });
    }
    /**
