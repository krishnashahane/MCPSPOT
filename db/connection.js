...
const createRequiredExtensions = async (dataSource) => {
    try {
...
const ensureEmbeddingColumnIsVector = async (dataSource) => {
...
const performDatabaseInitialization = async () => {
    try {
...
export const startHealthCheck = () => {
    ...
    console.log(`[DB Health] Starting health check (interval: ${CONNECTION_CONFIG.healthCheckIntervalMs}ms)`);
...
const attemptReconnection = async () => {
    ...
    try {
        ...
        for (let attempt = 1; attempt <= CONNECTION_CONFIG.maxConnectionRetries; attempt++) {
            try {
                console.log(`[DB Reconnect] Connection attempt ${attempt}/${CONNECTION_CONFIG.maxConnectionRetries}...`);
                appDataSource = await updateDataSourceConfig();
        ...
        console.error(`[DB Reconnect] Failed to reconnect after ${CONNECTION_CONFIG.maxConnectionRetries} attempts`);
