...
description: "Understand MCPSpot's system architecture and design principles"
---

## System Overview

MCPSpot is designed as a scalable, modular platform for managing Model Context Protocol (MCP) servers. The architecture follows modern web application patterns with clear separation of concerns, microservices-ready design, and extensibility in mind.
...
class MetricsService {
  ...
  private initializeMetrics(): void {
    ...
    this.responseTime = new Histogram({
      name: 'mcpspot_response_time_seconds',
...
// Plugin manager
class PluginManager {
  private plugins: Map<string, MCPSpotPlugin> = new Map();

  async loadPlugin(plugin: MCPSpotPlugin): Promise<void> {
...
RUN adduser -S mcpspot -u 1001
WORKDIR /app
COPY --from=builder --chown=mcpspot:nodejs /app .
USER mcpspot
EXPOSE 3000
...
resource "aws_ecs_cluster" "mcpspot" {
  name = "mcpspot-cluster"
}

resource "aws_ecs_service" "mcpspot" {
  name            = "mcpspot"
  cluster         = aws_ecs_cluster.mcpspot.id
  task_definition = aws_ecs_task_definition.mcpspot.arn
  desired_count   = 3

  load_balancer {
    target_group_arn = aws_lb_target_group.mcpspot.arn
