...
MCPSpot makes it easy to manage and scale multiple MCP (Model Context Protocol) servers by organizing them into flexible Streamable HTTP (SSE) endpoints—supporting access to all servers, individual servers, or logical server groups.
...
Get MCPSpot running in minutes with Docker:

```bash
docker run -p 3000:3000 krishnashahane/mcpspot
```

Or with custom configuration:

```bash
docker run -p 3000:3000 -v $(pwd)/mcp_settings.json:/app/mcp_settings.json krishnashahane/mcpspot
```
...
MCPSpot provides multiple ways to access your MCP servers:
...
<CardGroup cols={2}>
  <Card title="Quick Start Guide" icon="rocket" href="/quickstart">
    Get MCPSpot running in 5 minutes
  </Card>
...
<CardGroup cols={3}>
  <Card title="GitHub" icon="github" href="https://github.com/krishnashahane/mcpspot">
    Source code and issue tracking
  </Card>
  <Card title="Discord" icon="discord" href="https://discord.gg/qMKNsn5Q">
    Join our community discussions
  </Card>
  <Card title="Sponsor" icon="heart" href="https://ko-fi.com/krishnashahane">
