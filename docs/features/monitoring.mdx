...
description: 'Monitor your MCP servers and analyze system logs with MCPSpot'
---

## Overview

MCPSpot provides comprehensive monitoring and logging capabilities to help you track server performance, debug issues, and maintain system health.

## Real-time Monitoring

### Server Status Dashboard

The MCPSpot dashboard provides real-time monitoring of all registered MCP servers:
...
MCPSpot supports standard log levels:
...
# Log file location
LOG_FILE=/var/log/mcpspot/app.log
...
MCPSpot uses structured logging for better analysis:
...
ELASTICSEARCH_INDEX=mcpspot-logs
```
...
MCPSpot collects various system metrics:
...
```json
{
  "webhooks": [
    {
      "url": "https://your-service.com/webhooks/mcpspot",
...
Import MCPSpot Grafana dashboard:

```json
{
  "dashboard": {
    "title": "MCPSpot Monitoring",
    "panels": [
      {
        "title": "Server Status",
        "type": "stat",
      ...
      },
      {
        ...
        "targets": [
          {
            "expr": "rate(mcpspot_requests_total[5m])",
...
ls -la /var/log/mcpspot/

# Verify disk space
df -h /var/log/

# Check log rotation
logrotate -d /etc/logrotate.d/mcpspot
```
...
```bash
# Enable debug mode
DEBUG=mcpspot:* npm start

# Or set environment variable
export DEBUG=mcpspot:monitoring,mcpspot:logging
```
