...
## When MCPSpot Switches to OAuth
1. MCPSpot calls an MCP server that requires authorization and receives `401 Unauthorized`.
2. The response exposes a `WWW-Authenticate` header pointing to protected resource metadata (`authorization_server` or `as_uri`).
3. MCPSpot discovers the authorization server metadata, registers (if needed), and opens the browser so the user can authorize once.
4. After the callback is handled, MCPSpot reconnects with fresh tokens and resumes requests transparently.

> MCPSpot logs each stage (discovery, registration, authorization URL, token exchange) in the server detail view and the backend logs.
...
- MCPSpot discovers the authorization server, registers the client, and handles PKCE automatically.
...
4. Supply the credentials through the MCPSpot dashboard or by editing `mcp_settings.json` as shown below.
...
- MCPSpot skips dynamic registration and uses the credentials you provide to complete the OAuth exchange.
...
```
- MCPSpot will discover the authorization server from challenge headers and walk the user through authorization automatically.
    ...
    "dynamicRegistration": {
      ...
      "metadata": {
        ...
        "redirect_uris": [
          ...
          "https://mcpspot.example.com/oauth/callback"
        ],
...
```
- MCPSpot discovers endpoints via `issuer`, registers itself, and persists the issued `client_id`/`client_secret`.
- Provide `initialAccessToken` only when the registration endpoint is protected.

## Authorization Flow
1. **Initialization** – On startup MCPSpot processes every server entry, discovers metadata, and registers the client if `dynamicRegistration.enabled` is true.
...
- Remove stale tokens from `mcp_settings.json` if an authorization server revokes access—MCPSpot will prompt for a fresh login on the next request.
