...
description: 'Configuring MCPSpot to use a PostgreSQL database as an alternative to the mcp_settings.json file.'
---

# Database Configuration for MCPSpot

## Overview

MCPSpot supports storing configuration data in a PostgreSQL database as an alternative to the `mcp_settings.json` file. Database mode provides enhanced persistence and scalability for production environments and enterprise deployments.
...
<Note>
**Simplified Configuration**: You only need to set `DB_URL` to enable database mode. MCPSpot will automatically detect and enable database mode when `DB_URL` is present. Use `USE_DB=false` to explicitly disable database mode even when `DB_URL` is set.
</Note>
...
docker run -d \
  ...
  -e DB_URL="postgresql://user:password@your-db-host:5432/mcpspot" \
  krishnashahane/mcpspot
```
...
services:
  ...
  mcpspot:
    ...
    environment:
      USE_DB: "true"
      DB_URL: "postgresql://mcpspot:your_secure_password@postgres:5432/mcpspot"
      PORT: 3000
    ports:
      - "3000:3000"
    depends_on:
...
CREATE USER mcpspot WITH ENCRYPTED PASSWORD 'your_password';
GRANT ALL PRIVILEGES ON DATABASE mcpspot TO mcpspot;
EOF
```

#### Step 2: Install MCPSpot
...
Or let MCPSpot auto-migrate on first startup.

#### Step 5: Start MCPSpot
...
MCPSpot provides automatic migration on first startup when database mode is enabled. However, you can also run the migration manually.

### Automatic Migration

When you start MCPSpot with `USE_DB=true` for the first time:

1. MCPSpot connects to the database
...
MCPSpot creates the following tables:
...
```bash
# PostgreSQL backup
pg_dump -U mcpspot mcpspot > mcpspot_backup.sql

# Or using Docker
docker exec postgres pg_dump -U mcpspot mcpspot > mcpspot_backup.sql
```
...
```bash
# PostgreSQL restore
psql -U mcpspot mcpspot < mcpspot_backup.sql

# Or using Docker
docker exec -i postgres psql -U mcpspot mcpspot < mcpspot_backup.sql
```
...
2. Restart MCPSpot
3. MCPSpot will use `mcp_settings.json` again
...
```
Error: password authentication failed for user "mcpspot"
```
...
2. **Network Access:** Restrict database access to MCPSpot instances only
3. **Encryption:** Use SSL/TLS for database connections in production:
   ```bash
   DB_URL=postgresql://user:password@host:5432/mcpspot?sslmode=require
   ```
