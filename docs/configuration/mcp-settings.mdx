...
description: 'Configure MCP servers and their settings for MCPSpot'
---

# MCP Settings Configuration

This guide explains how to configure MCP servers in MCPSpot using the `mcp_settings.json` file and related configurations.

## Configuration Files Overview

MCPSpot uses several configuration files:
...
MCPSpot supports environment variable substitution using `${VAR_NAME}` syntax:
...
MCPSpot supports routing STDIO server network traffic through a proxy using **proxychains4**. This feature is available on **Linux and macOS only** (Windows is not supported).
...
```json
{
  "servers": {
    "fetch": {
      ...
      "version": "1.0.0",
      "author": "MCPSpot Team",
      "documentation": "https://docs.mcpspot.com/servers/fetch",
      "enabled": true
    },
    "playwright": {
...
MCPSpot supports hot reloading of configurations:
...
MCPSpot validates configurations on startup and reload:
...
This comprehensive guide covers all aspects of configuring MCP servers in MCPSpot for various use cases and environments.
