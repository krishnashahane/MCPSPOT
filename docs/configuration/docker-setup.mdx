---
title: 'Docker Setup'
description: 'Deploy MCPSpot using Docker and Docker Compose'
---

# Docker Setup

This guide covers deploying MCPSpot using Docker, including development and production configurations.
...
git clone https://github.com/your-username/mcpspot.git
cd mcpspot

# Build the Docker image
docker build -t mcpspot:local .
...
docker build --build-arg INSTALL_EXT=true -t mcpspot:extended .
...
docker exec mcpspot docker --version
docker exec mcpspot docker ps
```
...
services:
  mcpspot:
    ...
    environment:
      ...
      - DB_URL=postgresql://mcpspot:password@postgres:5432/mcpspot
    volumes:
      - ./mcp_settings.json:/app/mcp_settings.json:ro
      - ./servers.json:/app/servers.json:ro
      - mcpspot_data:/app/data
    depends_on:
      postgres:
  ...
  postgres:
    ...
    container_name: mcpspot-postgres
    environment:
      - POSTGRES_DB=mcpspot
    ...
    ports:
      - '5432:5432'
    healthcheck:
      test: ['CMD-SHELL', 'pg_isready -U mcpspot -d mcpspot']
      interval: 10s
      timeout: 5s
      retries: 5
    restart: unless-stopped
...
services:
  ...
  mcpspot:
    ...
    environment:
      ...
      - DB_URL=postgresql://mcpspot:${POSTGRES_PASSWORD}@postgres:5432/mcpspot
...
services:
  ...
  postgres:
    image: pgvector/pgvector:pg17
    container_name: mcpspot-postgres-dev
    environment:
      - POSTGRES_DB=mcpspot
      - POSTGRES_USER=mcpspot
      - POSTGRES_PASSWORD=password
...
# View logs
docker-compose -f docker-compose.dev.yml logs -f mcpspot-dev
...
```bash
#!/bin/sh
BACKUP_FILE="/backups/mcpspot_$(date +%Y%m%d_%H%M%S).sql"
pg_dump -h postgres -U mcpspot -d mcpspot > "$BACKUP_FILE"
echo "Backup created: $BACKUP_FILE"

# Keep only last 7 days of backups
find /backups -name "mcpspot_*.sql" -mtime +7 -delete
```
...
services:
  watchtower:
    ...
    container_name: mcpspot-watchtower
    volumes:
...
**Container fails to start**: Check logs with `docker-compose logs mcpspot`
...
docker-compose exec mcpspot sh

# Check database connection
docker-compose exec postgres psql -U mcpspot -d mcpspot

# Restart specific service
docker-compose restart mcpspot
...
This Docker setup provides a complete containerized environment for MCPSpot with development and production configurations.
