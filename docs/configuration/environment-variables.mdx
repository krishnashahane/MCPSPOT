...
description: 'Configure MCPSpot using environment variables'
---

# Environment Variables

MCPSpot uses environment variables for configuration. This guide covers all available variables and their usage.
...
| `MCPHUB_SETTING_PATH` | | Path to the MCPSpot settings |
...
| `AZURE_OPENAI_EMBEDDING_MODEL` | `string` | `text-embedding-3-small` | The actual OpenAI model name deployed in Azure (e.g. `text-embedding-3-small`). Azure deployment names are arbitrary identifiers, so this field tells MCPSpot which OpenAI model is behind the deployment, enabling correct token-limit enforcement and tokenizer selection. Only used when `SMART_ROUTING_EMBEDDING_PROVIDER=azure_openai`. |
...
MCPSpot loads environment variables in the following order:
...
MCPSpot supports variable expansion:
