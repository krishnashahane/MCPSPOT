...
MCPSpot now supports generating OpenAPI 3.0.3 specifications from MCP tools, enabling seamless integration with OpenWebUI and other OpenAPI-compatible systems without requiring MCPO as an intermediary proxy.
...
To integrate MCPSpot with OpenWebUI:

<Steps>
  <Step title="Start MCPSpot">
    Ensure MCPSpot is running with your MCP servers configured
  </Step>
  <Step title="Get OpenAPI Specification">
    ```bash
    curl http://localhost:3000/api/openapi.json > mcpspot-api.json
    ```
  </Step>
...
In OpenWebUI, you can add MCPSpot as an OpenAPI tool by using:
...
<AccordionGroup>
  ...
  <Accordion title="OpenWebUI can't connect">
    Ensure MCPSpot is accessible from OpenWebUI and the OpenAPI URL is correct.
  </Accordion>
